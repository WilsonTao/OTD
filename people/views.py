...
def login_twitter(request):
    ...
    '''
    return redirect(reverse('user_profile_form'))
...
def python_group_crud(request, pk=None):
    ...
    try:
        ...
        if not group.is_group_owner(request.user):
            ...
            return redirect(reverse('python_group_list'))
    except:
        group = None
    ...
    if request.POST:
        if form.is_valid():
            ...
            if group==None and pk==None:
                group = PythonGroup.objects.filter(user=request.user).latest('date_add')
            return redirect(reverse('python_group_detail', args=[group.pk]))
        else:
...
def survey_crud(request, pk=None):
    ...
    try:
        ...
        if not survey.is_group_owner(request.user):
            survey = None
            messages.add_message(request, messages.INFO, 'You cannot update this python user group.')
    ...
    if request.POST:
        if form.is_valid():
            ...
            if survey==None and pk==None:
                survey = Survey.objects.filter(user=request.user).latest('date_add')
            return redirect(reverse('survey_detail', args=[survey.pk]))
        else:
            messages.add_message(request, messages.ERROR, 'There are erros in your request. Please check the messages below.')
    return render(request,
